...
Date: 16/10/2022
Update: 21/10/2022 the code have been adapted to the new project structure thx to librairy os
Usage: python3 01_ks_gen.py --n_items [number of items to generate] --capacity_ks [maximum capacity of knapsack]
input: number of item and the maximum weight (capacity)
...
def generate_knapsack_problem(n_items, capacity_ks):
    ...
    with open(os.path.join(os.path.dirname(__file__), '..', 'Input', "knapsack_instances.csv"), "w", newline='') as csv_file:
